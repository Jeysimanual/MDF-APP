...
public class MainActivity extends BaseActivity {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == 1) { // Notification permission request code
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                Log.d(TAG, "onRequestPermissionsResult: Notification permissions granted");
                Toast.makeText(this, "Notification permissions granted", Toast.LENGTH_SHORT).show();
            } else {
                Log.d(TAG, "onRequestPermissionsResult: Notification permissions denied");
                Toast.makeText(this, "Notification permissions are required for event alerts", Toast.LENGTH_LONG).show();
            }
        }
    }
}