...
public class TeacherInformation extends BaseActivity {
    ...
    private ImageView add_profile_teacher;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d(TAG, "Firebase initialized. Database path: " + teachersRef.toString());
    ...
    private void initializeUI() {
        try {
            ...
            add_profile_teacher = findViewById(R.id.add_profile_teacher);

            // Profile image click listener
            add_profile_teacher.setOnClickListener(v -> {
                Intent intent = new Intent(Intent.ACTION_PICK, MediaStore.Images.Media.EXTERNAL_CONTENT_URI);
                intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
                pickImage.launch(intent);
            });

            // Back button click listener if exists
            ...
            result -> {
                if (result.getResultCode() == RESULT_OK && result.getData() != null) {
                    ...
                    try {
                        ...
                        Toast.makeText(TeacherInformation.this, "Uploading profile image...", Toast.LENGTH_SHORT).show();
                        ...
                        Toast.makeText(TeacherInformation.this, "Error loading image", Toast.LENGTH_SHORT).show();
                    }
                }
            }
    );
    ...
    private void uploadImageToFirebase(Uri imageUri) {
        ...
        Log.d(TAG, "Starting image upload for teacher: " + currentUserId);

        // Create a storage reference specifically for the teacher_profiles collection
        StorageReference fileRef = storageRef.child("teacher_profiles/" + currentUserId + "/profile_image.jpg");
        ...
        fileRef.putFile(imageUri)
                .addOnSuccessListener(taskSnapshot -> {
                    ...
                    fileRef.getDownloadUrl().addOnSuccessListener(uri -> {
                        ...
                        // Save URL to the new teacher_profiles collection
                        saveImageUrlToNewCollection(imageUrl);
                    }).addOnFailureListener(e -> {
                        imageUploading = false;
                        Log.e(TAG, "Failed to get download URL: " + e.getMessage());
                        Toast.makeText(TeacherInformation.this, "Failed to process uploaded image", Toast.LENGTH_SHORT).show();
                    });
                })
                .addOnFailureListener(e -> {
                    imageUploading = false;
                    Log.e(TAG, "Upload failed: " + e.getMessage());
                    Toast.makeText(TeacherInformation.this, "Failed to upload image", Toast.LENGTH_SHORT).show();
                })
    ...
    private void saveImageUrlToNewCollection(String imageUrl) {
        ...
        // Create a completely new reference to a teacher_profiles collection
        DatabaseReference profilesRef = FirebaseDatabase.getInstance().getReference().child("teacher_profiles");
        ...
        if (user != null) {
            ...
            // Also add any available teacher info
            teachersRef.child(currentUserId).addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    if (dataSnapshot.exists()) {
                        // Add teacher info to profile data if available
                        ...
                        if (dataSnapshot.hasChild("role")) {
                            profileData.put("role", dataSnapshot.child("role").getValue(String.class));
                        }
                    }
                ...
                public void onCancelled(@NonNull DatabaseError error) {
                    // Continue saving even if we can't get teacher data
                    saveProfileData(profilesRef, profileData);
                }
            });
        } else {
    ...
    private void saveProfileData(DatabaseReference profilesRef, Map<String, Object> profileData) {
        // Save profile data to the teacher_profiles collection
        profilesRef.child(currentUserId).setValue(profileData)
                .addOnSuccessListener(aVoid -> {
                    Log.d(TAG, "✅ Profile data successfully saved to teacher_profiles collection");

                    // Update the main teachers collection with the profile image URL
                    updateTeacherProfileImage((String) profileData.get("profileImage"));
                })
                .addOnFailureListener(e -> {
                    imageUploading = false;
                    Log.e(TAG, "Failed to save profile data: " + e.getMessage());
                    Toast.makeText(TeacherInformation.this, "Failed to update profile", Toast.LENGTH_SHORT).show();
                });
    }

    private void updateTeacherProfileImage(String imageUrl) {
        if (imageUrl == null || imageUrl.isEmpty()) {
            imageUploading = false;
            return;
        }

        // Update the teacher's record with the profile image URL
        teachersRef.child(currentUserId).child("profileImage").setValue(imageUrl)
                .addOnSuccessListener(aVoid -> {
                    imageUploading = false;
                    Log.d(TAG, "✅ Profile image URL successfully saved to teachers collection");
                    Toast.makeText(TeacherInformation.this, "Profile image updated successfully", Toast.LENGTH_SHORT).show();
                    loadProfileImage(imageUrl);
                })
                .addOnFailureListener(e -> {
                    imageUploading = false;
                    Log.e(TAG, "Failed to update teacher profile image: " + e.getMessage());
                    Toast.makeText(TeacherInformation.this, "Failed to update profile in database", Toast.LENGTH_SHORT).show();
                });
    }
    ...
    private void loadCurrentUserData() {
        ...
        if (userEmailStr != null) {
            findTeacherByEmail(userEmailStr, currentUserId);
        } else {
            // Fallback to direct UID lookup
            findTeacherByUid(currentUserId);
        }

        // Also check the teacher_profiles collection for profile image
        checkTeacherProfiles(currentUserId);
    }

    private void checkTeacherProfiles(String uid) {
        DatabaseReference profilesRef = FirebaseDatabase.getInstance().getReference().child("teacher_profiles").child(uid);
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists() && dataSnapshot.hasChild("profileImage")) {
                    ...
                    if (profileImageUrl != null && !profileImageUrl.isEmpty()) {
                        Log.d(TAG, "Found profile image in teacher_profiles collection: " + profileImageUrl);
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    for (DataSnapshot teacherSnapshot : dataSnapshot.getChildren()) {
                        ...
                        if (teacherUid != null) {
                            currentUserId = teacherUid; // Update the current user ID
                ...
                try {
                    if (dataSnapshot.exists()) {
                        ...
                        if (teacher != null) {
                            ...
                            // Load profile image if available (fallback)
                            if (dataSnapshot.hasChild("profileImage")) {
                                String profileImageUrl = dataSnapshot.child("profileImage").getValue(String.class);
                                if (profileImageUrl != null && !profileImageUrl.isEmpty()) {
                                    loadProfileImage(profileImageUrl);
