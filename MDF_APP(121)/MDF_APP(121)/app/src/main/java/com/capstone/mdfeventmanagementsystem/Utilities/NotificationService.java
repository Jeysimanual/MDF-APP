...
public class NotificationService extends Service {
    ...
    private void listenForEventProposals() {
        ...
        // Retrieve the teacherId from SharedPreferences
        ...
        // Reference to eventProposals and events collections
        DatabaseReference eventProposalsRef = FirebaseDatabase.getInstance().getReference("eventProposals");
        DatabaseReference eventsRef = FirebaseDatabase.getInstance().getReference("events");

        // Listen for new events in the events collection to detect approved proposals
        eventsRef.addChildEventListener(new ChildEventListener() {
            @Override
            public void onChildAdded(@NonNull DataSnapshot snapshot, String previousChildName) {
                String eventId = snapshot.getKey();
                ...
                if (eventId == null || eventName == null) {
                    Log.d(TAG, "Event approval check skipped: Missing eventId or eventName.");
                    return;
                }

                // Check if this eventId exists in eventProposals and was created by the current teacher
                eventProposalsRef.child(eventId).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot proposalSnapshot) {
                        if (proposalSnapshot.exists()) {
                            ...
                            String proposalNotifyKey = "proposalApproved_" + eventId;
                            boolean alreadyNotified = sharedPreferences.getBoolean(proposalNotifyKey, false);

                            if (createdBy != null && createdBy.equals(teacherId) && !alreadyNotified) {
                                Log.d(TAG, "Event proposal approved for event: " + eventName);
                                sendNotification("Event Proposal Approved", "Your proposed event \"" + eventName + "\" has been approved!");
                                sharedPreferences.edit().putBoolean(proposalNotifyKey, true).apply();
                                // Clear rejection/restoration status to allow future notifications
        ...
        // Listen for status changes in eventProposals
        eventProposalsRef.addChildEventListener(new ChildEventListener() {
            @Override
            public void onChildAdded(@NonNull DataSnapshot snapshot, String previousChildName) {
                // Store initial status as "pending" for new proposals
                String eventId = snapshot.getKey();
                String status = snapshot.child("status").getValue(String.class);
                if (eventId != null && status != null) {
                    sharedPreferences.edit().putString("lastStatus_" + eventId, status).apply();
                }
            }
            ...
            public void onChildChanged(@NonNull DataSnapshot snapshot, String previousChildName) {
                ...
                if (eventId == null || eventName == null || status == null || createdBy == null) {
                    Log.d(TAG, "Proposal status check skipped: Missing data.");
                    return;
                }

                // Only process notifications for proposals created by the current teacher
                ...
                String rejectionNotifyKey = "proposalRejected_" + eventId;
                String restorationNotifyKey = "proposalRestored_" + eventId;
                boolean alreadyRejected = sharedPreferences.getBoolean(rejectionNotifyKey, false);
                boolean alreadyRestored = sharedPreferences.getBoolean(restorationNotifyKey, false);

                // Update last known status
                sharedPreferences.edit().putString(lastStatusKey, status).apply();

                // Handle rejection (from pending to rejected)
                if (status.equalsIgnoreCase("rejected") && lastStatus.equalsIgnoreCase("pending") && !alreadyRejected) {
                    Log.d(TAG, "Event proposal rejected for event: " + eventName);
                    sendNotification("Event Proposal Rejected", "Your proposed event \"" + eventName + "\" has been rejected.");
                    sharedPreferences.edit().putBoolean(rejectionNotifyKey, true).apply();
                }
                // Handle restoration (from rejected to pending)
                else if (status.equalsIgnoreCase("pending") && lastStatus.equalsIgnoreCase("rejected") && !alreadyRestored) {
                    Log.d(TAG, "Event proposal restored for event: " + eventName);
                    sendNotification("Event Proposal Restored", "Your proposed event \"" + eventName + "\" has been restored to pending status.");
                    sharedPreferences.edit().putBoolean(restorationNotifyKey, true).apply();
                }
            }
