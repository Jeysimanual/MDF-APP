...
public class TeacherProfile extends BaseActivity {
    ...
    private void logoutUser() {
        ...
        // 1. Get the teacherId from SharedPreferences
        SharedPreferences prefs = getSharedPreferences("UserSession", MODE_PRIVATE);
        String teacherId = prefs.getString("teacherId", null);

        // 2. REMOVE FCM TOKEN FROM REALTIME DATABASE (CRITICAL!)
        if (teacherId != null) {
            FirebaseDatabase.getInstance()
                    ...
                    .addOnSuccessListener(aVoid -> {
                        ...
                        Toast.makeText(TeacherProfile.this, "Logged out safely", Toast.LENGTH_SHORT).show();
                    })
                    .addOnFailureListener(e -> {
                        Log.e("Logout", "Failed to remove teacher FCM token", e);
                        // Still continue logout even if cleanup fails
                    });
        } else {
            Log.w("Logout", "teacherId not found in SharedPreferences");
        }
