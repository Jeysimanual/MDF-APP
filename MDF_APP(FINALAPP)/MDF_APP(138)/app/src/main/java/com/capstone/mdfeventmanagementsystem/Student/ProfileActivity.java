...
public class ProfileActivity extends BaseActivity {
    ...
    private void logoutUser() {
        ...
        // 1. Get the REAL studentID from SharedPreferences (NOT Auth UID!)
        SharedPreferences prefs = getSharedPreferences("UserSession", MODE_PRIVATE);
        String studentID = prefs.getString("studentID", null);

        // 2. REMOVE FCM TOKEN FROM THE CORRECT PLACE: /students/{studentID}/fcmToken
        if (studentID != null) {
            FirebaseDatabase.getInstance()
                    ...
                    .addOnSuccessListener(aVoid -> {
                        Log.d("Logout", "FCM token SUCCESSFULLY REMOVED from /students/" + studentID + "/fcmToken");
                        Toast.makeText(ProfileActivity.this, "Logged out safely", Toast.LENGTH_SHORT).show();
                    })
                    .addOnFailureListener(e -> {
                        Log.e("Logout", "Failed to remove FCM token from database", e);
                    });
        } else {
            Log.w("Logout", "studentID is NULL — cannot remove FCM token");
        }
        ...
        Log.d("ProfileActivity", "User logged out completely – FCM token cleaned from /students node");
